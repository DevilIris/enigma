...
- After changing web code: `npm run build && npm run cap:sync:ios`, commit, rebuild in Appflow.
  - ⚠️ **Use `npm run cap:sync:ios`, not raw `npx cap sync ios`, on Windows.** Capacitor's CLI writes the SPM manifest (`ios/App/CapApp-SPM/Package.swift`) with Windows **backslash** paths to `node_modules`, which are invalid on Appflow's macOS builder and make `xcodebuild` fail to resolve all 10 plugins. The `cap:sync:ios` script runs `scripts/fix-spm-paths.mjs` immediately after sync to rewrite those paths to POSIX forward slashes. If you ever run a bare `cap sync`, run `node scripts/fix-spm-paths.mjs` afterward and commit.
